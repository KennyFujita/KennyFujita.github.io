...
- **外部 API への通信** — 現在の通信先は次の2つだけ。増やすときはここに追記する
  ...
  **分類（NDC）も NDL からしか取れない。** `dcterms:subject` の `rdf:resource` に
  `http://id.ndl.go.jp/class/ndc9/548.31` の形で入っている。同じ要素に `class/ndlc/…`
  （NDL 独自分類）も並ぶので、**`class/ndc\d*/` で絞ること**。第1桁が大分類
...
- **グラフの配色を目分量で決めること** — 下記のとおり検証を通すこと

### グラフの色（`--cat-1` 〜 `--cat-5` ＋ `--cat-other`）

**背表紙の装丁色（`CLOTH`）をグラフに流用してはいけない。** 積み上がった本の質感のために
暗く彩度を落としてあるので、隣り合う扇を見分けられない。検証スクリプトにかけた結果:
...
- **ルール1（英日を対で更新）は、アプリのページ本体には適用しない** — `tools/` 配下に `/ja/` 版は
  作らない。**ただし `works.html` の紹介カードは英日ペアで書く**（カードは文書ページの一部なので
  ルール1が効く）。件数はこれで確認できる:

  ```sh
  grep -c '<article class="card">' works.html ja/works.html   # 一致すること
  ```

- **ルール2（ナビの一括更新）** — **`tools/` 配下をサイトのナビに載せない**。
  載せると全 HTML のヘッダー/フッターを書き換えることになる。
  入口は `works.html` の制作物カードで足りる
...
`tsundoku` は `works.html` / `ja/works.html` の制作物として掲載済み。切り替え時にやったこと:

1. `noindex` の meta を外した
2. **サンプル投入ボタン**を足した。初見の人は空の画面では何も分からないため。
   `sample: true` を付けて登録し、「サンプルを消す」でまとめて消せる。
   このボタンは**1冊も無いときだけ**出る（既に本がある人には見えない）
3. SEO 一式（`description` / Open Graph / canonical）。RM換算表と同水準
4. 英日ペアで紹介カードを追加

**新しい道具を `tools/` に足して制作物にするときも、この4点を同じようにやること。**
特に2番は忘れやすい。RM換算表は開いた瞬間に使えるが、状態を持つ道具は

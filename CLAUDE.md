...
**リンクの文言にタイトルを書かないこと。** 「限定公開のノート」/「Restricted notes」に
とどめる。ここに題名を書くと、鍵の内側に隠した意味が消える。個々のスライドへ
直接リンクするのも同じ理由で不可（URL に無意味な文字列が出るだけで実害は無いが、
入口を通らないとパスワードの引き継ぎが効かず、二度訊かれる）。

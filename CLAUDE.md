...
- **外部 API への通信** — 現在の通信先は次の2つだけ。増やすときはここに追記する
  ...
  | 通信先 | 何のため | いつ | 備考 |
  |---|---|---|---|
  | `api.openbd.jp` | ISBN から書名・著者 | ユーザーが ISBN を調べたときだけ | CORS 対応。キー不要 |
  | `www.googleapis.com/books` | ページ数の補完 | 同上 | キー無しの共有クォータですぐ 429 になる |

  **API キーを要求するサービスは足さない。** 公開ページにキーが載る上、リファラ制限をかけると
  ダウンロードして `file://` で開いた配布版が動かなくなる（持ち帰って使ってもらう方針と衝突する）。

  国立国会図書館サーチ（`ndlsearch.ndl.go.jp`）は **CORS ヘッダを返さないためブラウザから呼べない**。
  検証済みなので再検討しないこと。

  なお総ページ数は書誌データベースにほぼ入っていない（openBD で12冊中1冊）。**手入力が正しい仕様**で、
  自動取得を作り込もうとしないこと。

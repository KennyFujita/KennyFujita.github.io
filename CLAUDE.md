...
鍵付きにするかは1件ごとに決める（`slides/build.py` の `protected`）。鍵付きのものは
AES-256-CBC + HMAC-SHA256 で暗号化してあり、平文はリポジトリにも配信物にも入っていない。
...
`/slides/...` へのリンクを1行足すことだけ。書き分けは2通り:

| | 鍵なし | 鍵付き |
|---|---|---|
| 見出し | そのまま | 頭に 🔒（`&#128274;`） |
| 保存リンク | `<a class="dl" ... download>` を隣に置く | **置かない** |

**鍵付きの項目に保存リンクを付けないこと。** リンク先は平文になるので鍵の意味が消える。
鍵付きのものはパスワードを入れたあとのページ右下に保存ボタンが出る仕組みで、
「暗号化したまま誰でも落とせて、開くときだけパスワードを聞く」形は本人判断で採らなかった。
一覧冒頭の `.muted` で「🔒 はパスワードが要る」と断っている。
